...
void printUsage() {
    std::cout
        << "Usage: fastnet_http_connect_burst_benchmark [options]\n"
        << "  --port <value>            Listen port (default: 9340)\n"
...
public:
    ...
    void startConnect() {
        ...
        client_.setRequestTimeout(connectTimeoutMs_);
...
private:
    void handleConnect(bool success) {
        if (success) {
            ...
            sharedState_->failedClients.fetch_add(1, std::memory_order_relaxed);
            if (client_.getLastError().isFailure()) {
...
bool waitForRoundReady(const std::shared_ptr<SharedConnectState>& state, const BenchmarkOptions& options) {
...
ConnectRunResult runRound(FastNet::IoService& ioService, const BenchmarkOptions& options) {
    ...
    if (options.enableSsl) {
        auto generatedTlsFiles =
            std::make_unique<FastNetExamples::TempTlsFiles>(FastNetExamples::makeTempTlsFiles("http-connect-burst"));
    ...
    server.setServerErrorCallback([](const FastNet::Error& error) {
        std::cerr << "[http-connect-burst-server] " << error.toString() << '\n';
    });
    ...
    const std::string url = scheme + std::string("127.0.0.1:") + std::to_string(options.port);
...
void printRunResult(const ConnectRunResult& result, size_t roundIndex, size_t expectedClients) {
    ...
    std::cout << "  client errs : " << result.clientErrors << '\n';
}
...
int main(int argc, char** argv) {
    ...
    std::cout << "FastNet HTTP connect burst benchmark\n"
